...
def test_depleted_when_no_same_platform_mob(monkeypatch):
    # player feet at y=500; no mobs at all -> two empty reads -> DEPLETED
    _setup(monkeypatch, mobs=[], ptuple=(800, 500), clock_vals=[0, 0, 0])
    assert recovery.approach_shoot(10, [], None, 0.6, 0.5, None) is recovery.DEPLETED
...
def test_off_platform_mob_ignored_then_depleted(monkeypatch):
    ...
    assert recovery.approach_shoot(10, [], None, 0.6, 0.5, None, band=70) is recovery.DEPLETED
...
def test_walks_right_toward_far_mob(monkeypatch):
    ...
    recovery.approach_shoot(10, [], None, 0.6, 0.5, None, attack_range=110)
...
def test_blind_attack_when_anchor_lost(monkeypatch):
    ...
    recovery.approach_shoot(10, [], None, 0.6, 0.5, None)
