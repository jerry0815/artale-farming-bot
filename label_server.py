"""Minimal local box-labeling tool (no deps beyond Python stdlib).

Serves the curated dragon frames in a browser with their current YOLO boxes
pre-drawn; you fix them (draw / select+delete / adjust), and it writes corrected
YOLO labels back to datasets/dragons/labels/. Single class: dragon (id 0).

Run:   python label_server.py          # then open http://localhost:8000
Keys:  drag = new box | click box then Del/Backspace = remove | ArrowRight/Left =
       next/prev (auto-saves) | S = save now.  Progress persists in the .txt files.
"""
import json, os, glob, http.server, socketserver, urllib.parse

IMG_DIR = os.path.join("datasets", "dragons", "images")
LBL_DIR = os.path.join("datasets", "dragons", "labels")
POOL_DIR = os.path.join("datasets", "dragons", "_pool")
PORT = 8000
...
def drop_frame(stem):
    """Remove a frame from the set: move its image to _pool, delete its label."""
    import shutil
...
PAGE = """<!doctype html><html><head><meta charset=utf-8><title>dragon labeler</title>
<style>
...
<div id=bar>
 ...
 <span id=hint>drag = new box &middot; click box then Del = remove box &middot; X = drop whole frame &middot; arrows = prev/next (auto-save)</span>
</div>
...
if __name__ == "__main__":
    with socketserver.ThreadingTCPServer(("127.0.0.1", PORT), H) as srv:
        n = len(frames())
        print(f"labeling {n} frames -> http://localhost:{PORT}  (Ctrl+C to stop)")
        srv.serve_forever()
